...
for (let i = 1; i <= 1103; i++) {
  ...
  if (!ruQuestion || !ruRes.nextQuestion || !kkRes.nextQuestion || correct === -1 || ruRes.options.length < 2 || kkRes.options.length < 2) {
    console.log(`Issue at Q${i}:`, {
      hasRuQ: !!ruQuestion,
      hasKkQ: !!ruRes.nextQuestion,
      hasEnQ: !!kkRes.nextQuestion,
      correct,
      ruOpts: ruRes.options.length,
      kkOpts: kkRes.options.length
    });
    allValid = false;
  }
}

console.log('All 1103 questions valid:', allValid);
